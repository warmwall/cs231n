...
- 1e-7 혹은 그보다 작은 상대 오차라면, 행복함을 만끽하라.
...
**double precision형 변수를 사용하라 (Use double precision)**. A common pitfall is using single precision floating point to compute gradient check. It is often that case that you might get high relative errors (as high as 1e-2) even with a correct gradient implementation. In my experience I've sometimes seen my relative errors plummet from 1e-2 to 1e-8 by switching to double precision.
